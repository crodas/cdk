...
export class NativeOutputDataCreator implements OutputDataCreator {
  ...
  /**
   * NUT-09 restore outputs for a counter range, in one native call.
   *
   * Restore is the heaviest deterministic workload a wallet runs, so it gets a
   * direct entry point rather than one call per counter.
   */
  createRestoreData(seed: Uint8Array, keysetId: string, start: number, end: number): OutputDataLike[] {
    const native = withNativeErrors(() =>
      this.crypto.createRestoreOutputs(toArrayBuffer(seed), keysetId, start, end),
    );
    return native.map(toOutputData);
  }
}
