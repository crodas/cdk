...
```rust
let connector = PostgresBusConnector::connect(pg_config, "cdk_mint_events").await?;
...
Wiring the choice through `cdk-mintd` configuration is a follow-up; this ADR
covers the seam and the Postgres backend.
