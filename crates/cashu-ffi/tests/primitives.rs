...
use cashu_ffi::{
    blind_message, create_deterministic_outputs, create_random_outputs,
    create_single_deterministic_output, create_single_p2pk_output, create_single_random_output,
    hash_to_curve, keyset_id_v1, sha256_digest, split_amount, unblind_signature,
    verify_proof_dleq, CashuFfiError, DeterministicOutputFactory, DleqProof, KeyEntry, P2pkOptions,
    SigFlag,
};
...
fn deterministic_outputs_match_the_nut13_vectors() {
    ...
    for counter in 0..5u32 {
        let output =
            create_single_deterministic_output(1, seed(), counter, KEYSET_V0.to_string())
                .expect("derivation");
...
fn factory_and_free_function_agree() {
    let factory = DeterministicOutputFactory::new(seed(), KEYSET_V0.to_string())
        .expect("64 byte seed");
    let keys = powers_of_two_keys(6);

    let _ = keys;
    let from_object = factory.outputs(split_of(31), 3).expect("derivation");
    let from_function = create_deterministic_outputs(split_of(31), seed(), 3, KEYSET_V0.to_string())
        .expect("derivation");
...
fn caller_supplied_denominations_are_used_verbatim() {
    let outputs =
        create_random_outputs(vec![4, 2, 1, 1], KEYSET_V0.to_string()).expect("outputs");
...
fn split_amount_uses_the_supplied_denominations() {
    ...
    assert_eq!(split_amount(0, denominations, None).expect("split"), Vec::<u64>::new());
}
