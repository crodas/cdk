...
fn root_header(
    ...
    out.indented(|out| {
        ...
        out.indented(|out| { out.line("::{NS}::bridge::assertAbiCompatible();"); });
        out.line("}");
        out.blank();
    });
    ...
    out.line("};");
    out.blank();
    out.line(format!("}} // namespace margelo::nitro::{}", model.cxx_namespace));
...
fn emit_converters(out: &mut Source, model: &Model, wanted: &BTreeSet<String>) -> Result<()> {
    ...
    for enum_ in model.enums.iter().filter(|item| wanted.contains(&item.name)) {
        let bridge = format!("::{{NS}}::bridge::{}", enum_.name);
        out.line(format!(
            "{bridge} toBridge{0}({0} value);",
            enum_.name
        ));
    ...
    for record in model.records.iter().filter(|item| wanted.contains(&item.name)) {
        ...
        out.indented(|out| {
            ...
            for field in &record.fields {
                match to_bridge(&field.ty, &format!("value.{}", field.js_name)) {
                    Ok(expr) => out.line(format!("out.{} = {expr};", field.js_name)),
                    Err(_) => out.line("// unsupported field"),
                };
            }
            out.line("return out;");
        });
