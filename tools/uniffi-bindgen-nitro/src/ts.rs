...
pub fn spec(model: &Model, config: &NitroConfig, crate_name: &str) -> String {
    ...
    docs(&mut out, Some("Root native module. Every free function of the Rust crate hangs off it."));
    out.line(format!(
        "export interface {} extends {PLATFORMS} {{",
        model.module
    ));
...
pub fn errors(model: &Model, crate_name: &str) -> String {
    ...
    for error in &model.errors {
        ...
        out.indented(|out| {
            for (index, variant) in error.variants.iter().enumerate() {
                let sep = if index + 1 == error.variants.len() { ";" } else { "" };
                out.line(format!("| '{}'{sep}", variant.rust_name));
            }
        });
        out.blank();

        out.line(format!("/** A `{}` raised by the native module. */", error.name));
    ...
    out.indented(|out| {
        out.line("try {");
        out.indented(|out| { out.line("return call()"); });
        out.line("} catch (thrown) {");
        out.indented(|out| { out.line("throw toNativeError(thrown)"); });
        out.line("}");
    });
    out.line("}");
