...
pub fn source(model: &Model, crate_name: &str, header_name: &str, ffi_header: &str) -> Result<String> {
    let mut out = Source::new();
    out.lines(&banner("//", crate_name));
    out.blank();
...
fn emit_error_type(out: &mut Source, error: &EnumModel) -> Result<()> {
    ...
    out.line(format!("class {} final : public std::runtime_error {{", error.name));
    out.line("public:");
    out.indented(|out| {
        out.line(format!(
        ...
        out.blank();
        out.line(format!("{}Kind kind() const noexcept {{ return kind_; }}", error.name));
    });
    out.blank();
    out.line("private:");
    out.indented(|out| {
...
fn emit_record_declaration(out: &mut Source, record: &RecordModel) {
    ...
    out.indented(|out| {
        for field in &record.fields {
            out.line(format!("{} {};", field.mapped.bridge_local(), field.js_name));
        }
    });
    out.line("};");
    out.blank();
...
fn emit_object_declaration(out: &mut Source, model: &Model, object: &ObjectModel) {
    ...
    out.line("public:");
    out.indented(|out| {
        out.line(format!("explicit {}(uint64_t handle) noexcept : handle_(handle) {{}}", object.name));
        ...
        for method in &object.methods {
            ...
            out.line(format!("{};", declaration(model, method, Some(&object.name))));
        }
    });
    out.blank();
    out.line("private:");
...
fn emit_object_definition(out: &mut Source, model: &Model, object: &ObjectModel) -> Result<()> {
    ...
    out.indented(|out| {
        out.line("if (handle_ == 0) {");
        out.indented(|out| { out.line("return;"); });
        out.line("}");
        out.line("ffi::RustCallStatus status;");
        out.line("status.code = 0;");
        out.line("status.errorBuf = ffi::RustBuffer{0, 0, nullptr};");
...
fn signature(model: &Model, callable: &CallableModel, owner: Option<&ObjectModel>) -> String {
    signature_with(model, callable, owner.map(|object| object.name.as_str()), true)
}
...
fn emit_serializer_declarations(out: &mut Source, model: &Model) -> Result<()> {
    ...
    for (ser, (ty, mapped)) in &model.serialized {
        ...
        out.line(format!("void write{ser}(BufferWriter& writer, const {cpp}& value);"));
        if is_top_level(ty) {
    ...
    match ty {
        ...
        Type::Map { value_type, .. } => {
            ...
            out.indented(|out| {
                ...
                out.line(format!("entries.emplace(std::move(key), read{inner}(reader));"));
            });
            out.line("}");
...
fn emit_error_thrower(out: &mut Source, error: &EnumModel) -> Result<()> {
    out.line(format!("[[noreturn]] void throw{}(RustBuffer buffer) {{", error.name));
    out.indented(|out| {
        out.line("std::vector<uint8_t> bytes = consumeBuffer(buffer);");
        out.line("BufferReader reader(bytes.data(), bytes.size());");
        out.line("int32_t tag = reader.readI32();");
