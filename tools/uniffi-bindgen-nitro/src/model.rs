    ...
    fn collect_serialized(&mut self) -> Result<()> {
        ...
        for record in &self.records {
            for field in &record.fields {
                types.push((field.ty.clone(), format!("{}.{}", record.name, field.rust_name)));
            }
        }
        ...
        for callable in callables {
            for arg in &callable.args {
                types.push((arg.ty.clone(), format!("{}({})", callable.rust_name, arg.rust_name)));
            }
    ...
    for variant in enum_.variants() {
        if variant.has_nameless_fields() {
            return Err(Error::UnsupportedFeature {
                ...
                feature: "tuple variants (name the fields so JavaScript can read them)"
                    .to_string(),
            });
        }
        let mut fields = Vec::new();
