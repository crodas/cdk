            ...
            out.indented(|out| {
                if let Some(record) = record {
                    for field in &record.fields {
                        out.line(format!("{}: read{}(reader),", field.js_name, child(&field.ty)));
                    }
                }
            });
            out.line("}")
...
fn emit_declarations(out: &mut Source, model: &Model) {
    ...
    let all = model
        .root_methods
        .iter()
        .chain(model.objects.iter().flat_map(|object| object.methods.iter()));
    for callable in all {
    ...
    for (symbol, _) in &model.checksums {
        out.line(format!("fn['{symbol}'] = lib.func('{symbol}', 'uint16', [])"));
    }
    out.blank();
    out.line(format!(
...
fn emit_api(out: &mut Source, model: &Model) -> Result<()> {
    for object in &model.objects {
        ...
        body.indented(|body| {
            body.line("if (this.handle === 0) throw new Error('use of a native object after close()')");
